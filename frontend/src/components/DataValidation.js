...
const DataValidation = ({ filename, onDataUpdate }) => {
  ...
  const analyzeDataIntegrity = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/api/analyze-data-integrity');
  ...
  const handleAction = async (column, action) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5001/api/fix-data-integrity', payload);
